...
// // console.log('btnClip :', btnClip, 'clipList :', clipList, 'CNT_LIST :', CNT_LIST, '이동한계수(LIMIT_MOVE) :', LIMIT_MOVE);
...
// 맵쬬잉
// map() 메서드 리마인딩
// map()은 배열을 재구성하여 다시 같은 자리에 리턴하여 만들어짐
// 새로운 배열을 변수에 담거나 그 자리에 리턴한다.
// 이때 배열값을 문자열 값으로 변환하는 join()을 사용하여 연결자를 반값으로
// 처리하면 배열의 구분자 콤마가 없는 태그로만 연결된 순수한 태그 결과 문자열이 만들어진다.
...
console.log('subViewBox :', subViewBox, ', subContBox :', subContBox);
